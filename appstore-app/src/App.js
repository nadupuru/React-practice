...
class App extends Component{
  state={Activeid:buttonlist[0].id,
    search:""
  }
  setactiveid=(id)=>{
  ...
  getfiltereddata=()=>{
    ...
    const filterddata=projectdetails.filter(each=>each.category===Activeid && each.appName.toLowerCase().includes(search.toLowerCase()))
    return filterddata
  }
  render(){
    const filterddata=this.getfiltereddata()
    const{Activeid}=this.state
    return(
      <div className='container'>
        <h1 className='tittle'>App Store</h1>
        <input type='search' placeholder='search' className='search' onChange={this.changedata}/>
        <ul className='listcontainer'>
          {buttonlist.map(each=>
            <Button  buttonlist={each} key={each.id}
        ...
        <ul className='projectdata'>
          ...
          <Project projectdetails={each} key={each.appId}/>
          )}
